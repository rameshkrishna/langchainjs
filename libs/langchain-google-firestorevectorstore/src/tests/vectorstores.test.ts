...
describe("FirestoreVectorStore Unit Tests", () => {
  ...
  describe("Constructor and Initialization", () => {
    ...
    test("should return correct vectorstore type", () => {
      expect(firestoreVectorStore._vectorstoreType()).toBe("FirestoreVectorStore");
    });
  });
  ...
  describe("Document Operations", () => {
    describe("Adding Documents", () => {
      ...
      test("should add documents with provided IDs", async () => {
        ...
        const ids = await firestoreVectorStore.addDocuments(documents, { ids: providedIds });
    ...
    describe("Adding Vectors", () => {
      ...
      test("should add vectors with provided IDs", async () => {
        ...
        const resultIds = await firestoreVectorStore.addVectors(vectors, documents, { ids: providedIds });

        expect(resultIds).toEqual(["provided-id"]);
        expect(spy).toHaveBeenCalledWith(vectors, documents, { ids: providedIds });
    ...
    describe("Retrieving Documents", () => {
      test("should get document by ID", async () => {
        ...
        spy.mockResolvedValue(new Document({
      ...
      test("should return null for non-existent document", async () => {
        ...
        const document = await firestoreVectorStore.getDocumentById('non-existent');
      ...
      test("should get documents by metadata", async () => {
        ...
        const documents = await firestoreVectorStore.getDocumentsByMetadata({ type: 'test' });
      ...
      describe("Filter by Metadata Tests", () => {
        test("should filter documents by simple metadata field", async () => {
          ...
          const documents = await firestoreVectorStore.getDocumentsByMetadata(filter);
        ...
        test("should filter documents by multiple metadata fields", async () => {
          ...
          spy.mockResolvedValue([
            new Document({
              ...
              metadata: { category: 'tech', status: 'published', author: 'john' },
            }),
          ]);
        ...
        test("should handle complex nested dot notation filters", async () => {
          ...
          expect(documents).toHaveLength(1);
          expect(documents[0].metadata['user.profile.preferences.theme']).toBe('dark');
          expect(documents[0].metadata['settings.advanced.cache.enabled']).toBe(true);
          expect(spy).toHaveBeenCalledWith(filter);
        ...
        test("should handle metadata filter errors gracefully", async () => {
          const spy = jest.spyOn(firestoreVectorStore, 'getDocumentsByMetadata');
          spy.mockRejectedValue(new Error("Failed to get documents by metadata from Firestore."));

          const filter = { category: 'test' };
          
          await expect(
            firestoreVectorStore.getDocumentsByMetadata(filter)
          ).rejects.toThrow("Failed to get documents by metadata from Firestore.");
        ...
        test("should handle large metadata filter objects", async () => {
          ...
          const documents = await firestoreVectorStore.getDocumentsByMetadata(largeFilter);
        ...
        test("should handle special characters in metadata keys and values", async () => {
          ...
          expect(documents).toHaveLength(1);
          expect(documents[0].metadata['key-with-dashes']).toBe('value-with-dashes');
  ...
  describe("Search Operations", () => {
    test("should perform similarity search", async () => {
      const spy = jest.spyOn(firestoreVectorStore, 'similaritySearchVectorWithScore');
      spy.mockResolvedValue([
        [new Document({
      ...
      const results = await firestoreVectorStore.similaritySearchVectorWithScore(queryVector, 3);
    ...
    test("should return empty results when no matches found", async () => {
      ...
      const results = await firestoreVectorStore.similaritySearchVectorWithScore(queryVector, 5);
  ...
  describe("Error Handling", () => {
    test("should handle Firestore connection errors during document addition", async () => {
      ...
      await expect(firestoreVectorStore.addDocuments(documents)).rejects.toThrow(
        "Failed to add documents to Firestore."
      );
    ...
    test("should handle document retrieval errors", async () => {
      const spy = jest.spyOn(firestoreVectorStore, 'getDocumentById');
      spy.mockRejectedValue(new Error("Failed to get document by ID from Firestore."));

      await expect(firestoreVectorStore.getDocumentById('test-id')).rejects.toThrow(
        "Failed to get document by ID from Firestore."
      );
    ...
    test("should handle batch delete errors", async () => {
      ...
      await expect(firestoreVectorStore.delete({ deleteAll: true })).rejects.toThrow(
    ...
    describe("Similarity Search with Filters", () => {
      test("should perform similarity search with simple filter", async () => {
        ...
        const results = await firestoreVectorStore.similaritySearchVectorWithScore(
          queryVector, 
          5, 
          filter
        );
      ...
      test("should perform similarity search with multiple filters", async () => {
        ...
        spy.mockResolvedValue([
          [new Document({
            ...
            metadata: { category: 'science', status: 'published', author: 'expert' },
          }), 0.92],
        ]);
      ...
      test("should handle filter conflict with instance filter", async () => {
        ...
        const spy = jest.spyOn(filteredStore, 'similaritySearchVectorWithScore');
        spy.mockRejectedValue(new Error("Cannot provide both `filter` and `this.filter`"));

        const queryVector = await embeddings.embedQuery("test");
        const conflictingFilter = { category: 'tech' };

        await expect(
          filteredStore.similaritySearchVectorWithScore(queryVector, 5, conflictingFilter)
      ...
      test("should use instance filter when no filter provided", async () => {
        ...
        const results = await filteredStore.similaritySearchVectorWithScore(queryVector, 3);
      ...
      test("should perform similarity search with nested field filters", async () => {
        ...
        spy.mockResolvedValue([
          [new Document({
            ...
            metadata: { 'author.department': 'engineering', 'config.level': 'advanced' },
          }), 0.78],
        ]);
  ...
  describe("New Methods Tests", () => {
    ...
    describe("Enhanced Similarity Search", () => {
      test("should perform similarity search with filter", async () => {
        ...
        spy.mockResolvedValue([
          new Document({ pageContent: 'Filtered result', metadata: { type: 'test' } }),
        ]);
        ...
        expect(spy).toHaveBeenCalledWith("test query", 5, { filter: { type: 'test' } });

        spy.mockRestore();
      });

      test("should perform similarity search by vector", async () => {
        const spy = jest.spyOn(firestoreVectorStore, 'similaritySearchByVector');
        spy.mockResolvedValue([
          new Document({ pageContent: 'Vector search result', metadata: {} }),
        ]);

        const vector = [0.1, 0.2, 0.3];
        const results = await firestoreVectorStore.similaritySearchByVector(vector, 3);
      ...
      test("should perform similarity search by vector with filter", async () => {
        const spy = jest.spyOn(firestoreVectorStore, 'similaritySearchByVector');
        spy.mockResolvedValue([
          new Document({ pageContent: 'Filtered vector result', metadata: { category: 'A' } }),
        ]);
    ...
    describe("Maximal Marginal Relevance Search", () => {
      test("should perform MMR search", async () => {
        const spy = jest.spyOn(firestoreVectorStore, 'maxMarginalRelevanceSearchQuery');
        spy.mockResolvedValue([
          new Document({ pageContent: 'MMR result 1', metadata: {} }),
          new Document({ pageContent: 'MMR result 2', metadata: {} }),
        ]);

        const results = await firestoreVectorStore.maxMarginalRelevanceSearchQuery(
          "diverse query",
          2,
        ...
        expect(results[0]).toBeInstanceOf(Document);
        expect(spy).toHaveBeenCalledWith("diverse query", 2, { fetchK: 10, lambdaMult: 0.7 });

        spy.mockRestore();
      });

      test("should perform MMR search by vector", async () => {
        const spy = jest.spyOn(firestoreVectorStore, 'maxMarginalRelevanceSearchByVector');
        spy.mockResolvedValue([
          new Document({ pageContent: 'MMR vector result', metadata: {} }),
        ]);

        const vector = [0.2, 0.4, 0.6];
        const results = await firestoreVectorStore.maxMarginalRelevanceSearchByVector(
          vector,
          3,
      ...
      test("should handle MMR search with default parameters", async () => {
        ...
        const results = await firestoreVectorStore.maxMarginalRelevanceSearchQuery("test");
      ...
      test("should handle empty MMR search results", async () => {
        ...
        const results = await firestoreVectorStore.maxMarginalRelevanceSearchByVector(vector);
    ...
    describe("Utility Methods", () => {
      test("should convert Firestore vector to array", () => {
        ...
        expect(vectorStore._vectorToArray({ _values: [1, 2, 3] })).toEqual([1, 2, 3]);

        // Test with values format  
        expect(vectorStore._vectorToArray({ values: [4, 5, 6] })).toEqual([4, 5, 6]);
    ...
    describe("Error Handling for New Methods", () => {
      ...
      test("should handle similarity search errors", async () => {
        const spy = jest.spyOn(firestoreVectorStore, 'similaritySearch');
        spy.mockRejectedValue(new Error("Failed to perform similarity search."));
      ...
      test("should handle similarity search by vector errors", async () => {
        const spy = jest.spyOn(firestoreVectorStore, 'similaritySearchByVector');
        spy.mockRejectedValue(new Error("Failed to perform similarity search by vector."));
      ...
      test("should handle MMR search errors", async () => {
        const spy = jest.spyOn(firestoreVectorStore, 'maxMarginalRelevanceSearchQuery');
        spy.mockRejectedValue(new Error("Failed to perform maximal marginal relevance search."));

        await expect(
          firestoreVectorStore.maxMarginalRelevanceSearchQuery("test")
        ).rejects.toThrow("Failed to perform maximal marginal relevance search.");
      ...
      test("should handle MMR search by vector errors", async () => {
        const spy = jest.spyOn(firestoreVectorStore, 'maxMarginalRelevanceSearchByVector');
        spy.mockRejectedValue(new Error("Failed to perform maximal marginal relevance search by vector."));

        await expect(
          firestoreVectorStore.maxMarginalRelevanceSearchByVector([1, 2, 3])
        ).rejects.toThrow("Failed to perform maximal marginal relevance search by vector.");
  ...
  describe("Integration Behavior Tests", () => {
    test("should call addVectors when addDocuments is called", async () => {
      ...
      expect(addVectorsSpy).toHaveBeenCalledWith([[1, 2, 3]], documents, undefined);
