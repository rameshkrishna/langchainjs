...
class FirestoreVectorStore extends VectorStore {
  ...
  async similaritySearchVectorWithScore(
    ...
    try {
      ...
      // let coll: FirebaseFirestore.CollectionReference<
      //   FirebaseFirestore.DocumentData,
      //   FirebaseFirestore.DocumentData
      // > = this.firestore.collection(this.collectionName);

      if (_filter) {
        const filters = Object.keys(_filter).map((key) =>
          Filter.where(key, "==", _filter[key])
        );
        if (filters.length > 1) {
          baseQuery = baseQuery.where(Filter.and(...filters));
        } else if (filters.length === 1) {
          baseQuery = baseQuery.where(filters[0]);
        }
      }
